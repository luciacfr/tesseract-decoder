...
//Integer cost types used throughout the hot path.
// Change CostInt here to switch precision (int8_t → int16_t → int32_t).
using CostInt = int16_t;
using AccumInt = int64_t;

// Converts a floating-point log-odds cost to CostInt using the given scale.
// This is the single function to change when adjusting the integer representation.
inline CostInt float_to_cost(double x, double scale) {
  const double scaled = x * scale;
  const double lo = static_cast<double>(std::numeric_limits<CostInt>::min());
  const double hi = static_cast<double>(std::numeric_limits<CostInt>::max());
  const double clamped = scaled < lo ? lo : (scaled > hi ? hi : scaled);
  return static_cast<CostInt>(clamped + (clamped >= 0 ? 0.5 : -0.5));
}
