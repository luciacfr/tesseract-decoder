...
AccumInt TesseractDecoder::get_detcost(
    size_t d, const std::vector<DetectorCostTuple>& detector_cost_tuples) const {
  AccumInt min_cost = COST_INF;
  // Initialize to 1 (not UINT32_MAX) so early-exit cross-multiplication doesn't overflow
  // when min_cost == COST_INF. With doubles the original used UINT32_MAX safely because
  // finite * UINT32_MAX < INF, but with integers that product wraps.
  uint32_t min_det_cost = 1;
  AccumInt error_cost;
  ...
  for (int ei : d2e[d]) {
    ec = error_costs[ei];
// Cross-multiplication in int64 to avoid overflow.
    if ((int64_t)ec.likelihood_cost * min_det_cost >=
        (int64_t)min_cost * (int64_t)edets[ei].size()) break;

    dct = detector_cost_tuples[ei];
    if (!dct.error_blocked) {
      error_cost = static_cast<AccumInt>(ec.likelihood_cost);
      // After the first update min_cost < COST_INF (<=127), so product fits in int32.
      // Guard the initial COST_INF case to avoid overflow before that first update.
      if (min_cost >= COST_INF ||
          error_cost < min_cost * static_cast<AccumInt>(dct.detectors_count)) {
        min_cost = error_cost;
  ...
  if (min_cost >= COST_INF) return COST_INF;
    return (min_cost / static_cast<AccumInt>(min_det_cost)) + det_penalty_int;
}
...
TesseractDecoder::TesseractDecoder(TesseractConfig config_) : config(config_) {
  ...
  // Compute scale to map double log-odds into CostInt range.
  if (config.quant_auto_max) {
    double max_abs = 0.0;
    for (const auto& e : errors) {
      max_abs = std::max(max_abs, std::abs(e.likelihood_cost));
    }
    config.quant_max_abs = std::max(1e-6, max_abs * 1.1);
  }
  cost_scale = static_cast<double>(std::numeric_limits<CostInt>::max()) / config.quant_max_abs;
  det_penalty_int = static_cast<AccumInt>(std::round(config.det_penalty * cost_scale));
...
void TesseractDecoder::initialize_structures(size_t num_detectors) {
  ...
  for (size_t i = 0; i < errors.size(); ++i) {
    CostInt ic = float_to_cost(errors[i].likelihood_cost, cost_scale);
    int nd = static_cast<int>(errors[i].symptom.detectors.size());
    CostInt imc = (nd > 0) ? static_cast<CostInt>(ic / nd) : ic;
    error_costs.push_back({ic, imc});
  }
...
void TesseractDecoder::decode_to_errors(const std::vector<uint64_t>& detections) {
  ...
  AccumInt best_cost = std::numeric_limits<AccumInt>::max();
  if (config.det_orders.empty()) {
  ...
  if (config.beam_climbing) {
    ...
    for (int trial = 0; trial < std::max(config.det_beam + 1, int(config.det_orders.size()));
      ...
      AccumInt local_cost = cost_from_errors(predicted_errors_buffer);
  ...
  low_confidence_flag = best_cost == std::numeric_limits<AccumInt>::max();
}
...
void TesseractDecoder::decode_to_errors(const std::vector<uint64_t>& detections,
  ...
  AccumInt initial_cost = 0;
  for (size_t d : detections) {
    AccumInt dc = get_detcost(d, initial_detector_cost_tuples);
    if (dc >= COST_INF) { initial_cost = COST_INF; break; }
    initial_cost += dc;
  }

  if (initial_cost == COST_INF) {
  ...
  while (!pq.empty()) {
    ...
    std::vector<AccumInt> detector_cost_cache(num_detectors, -1);
    ...
    for (int ei : d2e[min_detector]) {
      ...
      AccumInt next_cost = node.cost + static_cast<AccumInt>(error_costs[ei].likelihood_cost);      size_t next_num_dets = node.num_dets;
      ...
      if (next_cost == COST_INF) continue;
...
AccumInt TesseractDecoder::cost_from_errors(const std::vector<size_t>& predicted_errors) const {
  AccumInt total_cost = 0;
  for (size_t dem_error_index : predicted_errors) {
    ...
    total_cost += static_cast<AccumInt>(error_costs[error_index].likelihood_cost);
  }
  return total_cost;
}
