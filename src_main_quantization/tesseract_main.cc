...
struct Args {
  ...
  size_t pqlimit;
    ...
    // Quantization
  int quant_bits = 0;
  double quant_max_abs = 20.0;
  bool quant_auto_max = false;
  ...
  void validate() {
    ...
    if (quant_bits < 0 || quant_bits > 16) {
      throw std::invalid_argument("quant-bits must be between 0 and 16");
    }
    if (quant_max_abs <= 0) {
      throw std::invalid_argument("quant-max-abs must be positive");
    }
  }
  ...
  void extract(TesseractConfig& config, std::vector<stim::SparseShot>& shots,
    ...
    config.quant_max_abs = quant_max_abs;
    config.quant_auto_max = quant_auto_max;
  }
};
...
int main(int argc, char* argv[]) {
  ...
  program.add_argument("--quant-bits")
      .help("Cuantiza likelihood_cost a N bits (0 desactiva)")
      .metavar("N")
      .default_value(0)
      .store_into(args.quant_bits);
  program.add_argument("--quant-max-abs")
      .help("Rango absoluto max esperado de log-odds para cuantizar (si no se usa auto)")
      .metavar("X")
      .default_value(20.0)
      .store_into(args.quant_max_abs);
  program.add_argument("--quant-auto-max")
      .help("Si se activa, toma max |likelihood_cost| observado del DEM para el rango")
      .flag()
      .store_into(args.quant_auto_max);
