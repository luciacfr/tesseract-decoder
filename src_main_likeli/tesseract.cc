...
// Cuantización uniforme simétrica de log-odds
double quantize_cost(double cost, int bits, double max_abs) {
  if (bits <= 0) return cost;
  double clamped = std::clamp(cost, -max_abs, max_abs);
  const double levels = std::pow(2.0, bits - 1) - 1.0;  // signed
  if (levels <= 0) return cost;
  double scale = levels / max_abs;
...
TesseractDecoder::TesseractDecoder(TesseractConfig config_) : config(config_) {
  ...
  errors = get_errors_from_dem(config.dem.flattened());
   ...
   // Ajuste automático del rango de cuantización si se solicita
  if (config.quant_bits > 0 && config.quant_auto_max) {
    double max_abs = 0.0;
    for (const auto& e : errors) {
      max_abs = std::max(max_abs, std::abs(e.likelihood_cost));
    }
    // Evita rango cero; añade pequeño margen
    config.quant_max_abs = std::max(1e-6, max_abs * 1.1);
  }
  // Aplicar cuantización uniforme
  if (config.quant_bits > 0) {
    for (auto& e : errors) {
      e.likelihood_cost = quantize_cost(e.likelihood_cost, config.quant_bits, config.quant_max_abs);
    }
  }
